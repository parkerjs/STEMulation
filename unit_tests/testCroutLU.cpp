...
// register factories...
static FactoryRegistrar<UnitTest> unit_test_factory("testCroutLU", &CroutLU_UnitTest::create);

/**
 * Constructor
 * @param dependencies a tuple of this object's required injection dependencies
 */
CroutLU_UnitTest::CroutLU_UnitTest(const tDependencies &dependencies)
 ...
 */
CroutLU_UnitTest *CroutLU_UnitTest::create(UnitTestManager *pUnitTestManager)
{
    CroutLU_UnitTest *pUnitTest = nullptr;
    if (pUnitTestManager != nullptr)
    {
        ...
        pUnitTest = new CroutLU_UnitTest(dependencies);
    }
 ...
 * Execution function
 */
bool CroutLU_UnitTest::execute(void)
{
    /*
     * Test Crout LU factorization
     */

    std::ofstream stream("outputs/croutLU_FactorTestOutput.dat");

    std::random_device seed;
    std::mt19937 generator(seed());
    std::normal_distribution<double> gaussian(5, 2);

    stream << " Test Crout LU factorization with partial pivoting:" << std::endl << std::endl;
    ...
    for (std::size_t i = 1; i <= 10; i++)
    {
        ...
        Crout_LU_Factor<decltype(LU)> solver;
        ...
        if (iErr >= 0)
        {
            ...
            stream << " Crout LU factorization:" << std::endl << std::endl;
        ...
        stream << " Test Crout LU factorization update:" << std::endl << std::endl;
        ...
        if (iErr >= 0)
        {
            ...
            stream << " Crout LU factorization without update:" << std::endl << std::endl;
        ...
        iErr = solver.factor(LU);
        if (iErr == -3)
            stream << " Matrix<2, double> is singular, cannot perform update."
                   << std::endl << std::endl;
        else
        {
            ...
            stream << " Crout LU factorization with rank-one update (Bennett's Algorithm):"
                   << std::endl << std::endl;
            ...
            if (iErr == 1)
            {
                ...
                diff = P * Aplus - L * U;
                stream << " Difference PA + Pxy' - { L * U | update(LU, x, y, p) }:"
                       << std::endl << std::endl;
                stream << std::setw(15) << std::setprecision(4) << diff << std::endl << std::endl;
            }
            else
        ...
        /*
         * Test inverse of matrix using Crout LU factorization
         */
        ...
        if (iErr != -3)
        {
            stream << " Inverse computed using Crout LU factorization:" << std::endl << std::endl;
        ...
        /*
         * Test determinant of a matrix using Crout LU decomposition
         */
        ...
        if (iErr >= 0)
        {
            ...
            if (iErr >= 0)
            {
                ...
                stream << " Determinant computed using Crout LU factorization: " << d
                       << std::endl << std::endl;
                stream << " Determinant of inverse computed using Crout LU factorization: "
        ...
        /*
         * Test solution to Ax = b using Crout LU decomposition
         */

        stream << " Test solution to Ax = b using Crout LU decomposition."
               << std::endl << std::endl;
